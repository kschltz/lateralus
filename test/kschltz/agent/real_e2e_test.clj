  ...
  (when-llm
   (testing "agent stores exchanges in Datalevin and retrieves them"
     (let [session-id (str "real-e2e-" (System/currentTimeMillis))
        ...
        ;; Memory store should exist immediately after make-agent
       (is (some? (core/get-memory-store ag)) "memory store should exist after make-agent")
       (let [p           (core/send-message! ag "My name is Zaphod Beeblebrox.")
             loop-future (future (core/start! ag))]
         @p
          ;; Memory store should still be alive during the loop
         (is (some? (core/get-memory-store ag)) "memory store should exist during loop")
         (Thread/sleep 1000)
         (let [conn    (core/get-memory-store ag)
               results (memory/retrieve-relevant
                        {:backend    :datalevin
                         ...
                         :query      "Zaphod"
                         :limit      5})]
           (is (some? conn) "memory store should exist before stop")
           (is (some? results) "should retrieve results from memory"))
         (when (core/running? ag) (core/stop! ag))
         @loop-future
          ;; Memory store should survive loop exit (not closed on stop)
         (is (some? (core/get-memory-store ag)) "memory store should survive loop exit")
         (core/reset! ag))))))
