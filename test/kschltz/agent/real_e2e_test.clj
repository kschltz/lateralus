  ...
  (when-llm
    (testing "start! processes queued messages via real LLM"
      (let [ag (core/make-agent {:base-url base-url
                                  :model    (ollama-model)
                                  :turns    2})]
        (let [p1 (core/send-message! ag "Say the word 'banana' and nothing else.")
              loop-future (future (core/start! ag))]
          (let [r1 (deref p1 30000 ::timeout)]
  ...
  (when-llm
    (testing "on-response handler receives real LLM response"
      (let [responses (atom [])
        ...
        (let [p (core/send-message! ag "Say hello.")
              loop-future (future (core/start! ag))]
          @p
          (Thread/sleep 500)
    ...
    (testing "per-message handler receives real LLM response"
      (let [handler-results (atom [])
            ag (core/make-agent {:base-url base-url
                                 :model    (ollama-model)
                                 :turns    2})]
        (let [p (core/send-message! ag "Say 'world'."
  ...
  (when-llm
    (testing "chat! builds history across multiple calls"
      (let [ag (core/make-agent {:base-url base-url
                                  :model    (ollama-model)
                                  :turns    5})]
        (let [r1 (core/chat! ag "Remember the number 42.")
              _   (await ag)
